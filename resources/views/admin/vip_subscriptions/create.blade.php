...
<head>
    ...
    <title>Dashboard - Thêm Đăng ký VIP</title>
...
<body>
    ...
    <div class="content">
        <div class="container">
            <h1>⭐ Thêm Đăng ký VIP</h1>

            <form action="{{ route('admin.vip_subscriptions.store') }}" method="POST">
                @csrf
                ...
                <div class="form-group">
                    ...
                    <select name="vip_package_id" id="vip_package_id" required>
                        ...
                        @foreach($vipPackages as $package)
                            <option value="{{ $package->id }}" {{ old('vip_package_id') == $package->id ? 'selected' : '' }}>{{ $package->name }} ({{ $package->duration }} ngày)</option>
                        @endforeach
                    </select>
                    @error('vip_package_id') <div class="error">{{ $message }}</div> @enderror
                </div>
